...
const AdsenseWidget = loadAsync(() => import("./AdsenseWidget.vue"));
...
const isSidebarEnabled = computed(() => {
  if (store.appConfig.showSidebar === false) return false;
...
const mainContentMaxWidth = computed(() => {
  ...
  const base = 1280;
  const maxAllowed = Math.round(width.value * 0.96);
      ...
      <div class="mx-auto transition-all duration-300" :style="{ maxWidth: mainContentMaxWidth }">
        ...
        >
          ...
          >
            ...
            >
              <p
                class="font-serif italic opacity-70 leading-tight"
                :class="store.appConfig.background ? 'text-white shadow-text' : 'text-gray-600'"
                style="font-size: 1.1em; max-width: 320px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;"
                :title="hitokoto.hitokoto"
              >
                “ {{ hitokoto.hitokoto }} ”
              </p>
              <p
                class="opacity-60 text-xs mt-0.5"
